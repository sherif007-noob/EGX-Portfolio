...
async function writeBars(sb: ReturnType<typeof supabase>, ticker: string, bars: HistoryBar[], startDate: string, endDate: string) {
  ...
  const filtered = bars.map((bar) => ({
    ticker, trading_date: toDate(Number(bar[0])), open: Number(bar[1]), high: Number(bar[2]), low: Number(bar[3]), close: Number(bar[4]),
...
async function main() {
  ...
  const sb = supabase();
  const { data: portfolio, error: portfolioError } = await sb.from('portfolios').select('id').eq('owner_key', ownerUid).maybeSingle();
  if (portfolioError) throw new Error(`Supabase portfolio lookup failed: ${portfolioError.message}`);
  if (!portfolio) throw new Error(`Supabase portfolio for owner ${ownerUid} does not exist.`);
  const { data: transactions, error: txError } = await sb.from('transactions').select('ticker,transaction_date').eq('portfolio_id', portfolio.id);
  if (txError) throw new Error(`Supabase transaction lookup failed: ${txError.message}`);
  const tickers = [...new Set((transactions ?? []).map((tx: any) => normalizeTicker(String(tx.ticker || ''))).filter((ticker) => ticker && ticker !== 'CASH'))];
  const firstTransactionDate = (transactions ?? []).map((tx: any) => String(tx.transaction_date || '').slice(0, 10)).filter(Boolean).sort()[0];
  ...
  try {
    ...
    let totalRows = 0; let totalExisting = 0;
