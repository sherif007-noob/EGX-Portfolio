...
function mapTransaction(row: any): TradeTransaction {
  return {
    ...
    cycleTag: row.cycle_tag ?? undefined, runningShares: row.running_shares == null ? undefined : Number(row.running_shares), grossTradeValue: row.gross_trade_value == null ? undefined : Number(row.gross_trade_value),
    netCashImpact: row.net_cash_impact == null ? undefined : Number(row.net_cash_impact), realizedPnlEgp: row.realized_pnl_egp == null ? undefined : Number(row.realized_pnl_egp), realizedPnlPercent: row.realized_pnl_percent == null ? undefined : Number(row.realized_pnl_percent),
    outcome: row.outcome ?? undefined, holdingDays: row.holding_days == null ? undefined : Number(row.holding_days), positionId: row.position_id ?? undefined,
  };
}
