      ...
      <MotionSwap motionKey={filterMode} variant="state" className="premium-journal-results space-y-3">
        {paginatedTransactions.map((tx) => {
          ...
          return (
            <div
              key={tx.id}
              className={`premium-card premium-semantic-edge premium-hierarchy-h5 premium-dense-row p-4 rounded-2xl border transition space-y-3 relative overflow-hidden ${
                isBuy
                  ? 'premium-glow-buy'
                  : isWinningSell
                  ? 'premium-glow-win'
